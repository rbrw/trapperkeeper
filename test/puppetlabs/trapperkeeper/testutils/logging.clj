...
;; Perhaps move with-started publically to kitchensink or some other namespace?

(defn- expand-with-started
  [[name init & bindings] body]
  (if (seq bindings)
    `(let [~name ~init]
       (try
         ~(expand-with-started bindings body)
         (finally (.stop ~name))))
    `(let [~name ~init]
       (try
         ~@body
...
(defmacro ^:private with-started
  ...
  [bindings & body]
  ;; We call out to a function because Clojure currently chokes on a
  ;; recursive private macro.
  (expand-with-started bindings body))
...
(defn call-with-log-event-listener
  "Calls (listen event) for each logger-id event produced during the
  evaluation of f.  If logger-id is not a class, it will be converted
  via str."
  [logger-id listen f]
...
(defmacro with-log-event-listener
  "Calls (listen event) for each logger-id event produced during the
  evaluation of body.  If logger-id is not a class, it will be
  converted via str."
  [logger-id listen & body]
  `(call-with-log-event-listener ~logger-id ~listen (fn [] ~@body)))
...
(defmacro with-log-suppressed-unless-notable
  ...
  [notable-event? & body]
  `(#'puppetlabs.trapperkeeper.testutils.logging/call-with-log-suppressed-unless-notable
    ~notable-event?
    (fn [] ~@body)))
...
(defmacro with-test-logging-debug
  ...
  `(let [destination# (atom [])]
     (binding [*test-log-events* destination#]
       (with-log-level root-logger-name :trace
         (with-log-event-listener root-logger-name
           (fn [event#]
             (binding [*out* *err*]
